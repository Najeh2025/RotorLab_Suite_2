...
REFERENCE_JSON = {
    'name': 'ROSS Tutorial Part 4',
    'material': {'name': 'Steel', 'rho': 7810, 'E': 211e9, 'G_s': 81.2e9},
    'rotor1': {
        'shaft': [
            {'L': 0.3, 'idl': 0.0, 'odl': 0.123, 'idr': 0.0, 'odr': 0.123},
            {'L': 0.092, 'idl': 0.0, 'odl': 0.15, 'idr': 0.0, 'odr': 0.15},
            {'L': 0.2, 'idl': 0.0, 'odl': 0.15, 'idr': 0.0, 'odr': 0.15},
            {'L': 0.2, 'idl': 0.0, 'odl': 0.15, 'idr': 0.0, 'odr': 0.15},
            {'L': 0.092, 'idl': 0.0, 'odl': 0.15, 'idr': 0.0, 'odr': 0.15},
            {'L': 0.3, 'idl': 0.0, 'odl': 0.123, 'idr': 0.0, 'odr': 0.123}
        ],
        ...
        'gear_elements': [
            {'n': 3, 'm': 14.37, 'Id': 0.068, 'Ip': 0.136, 'width': 0.07, 'n_teeth': 37, 'base_diameter': 0.19, 'pressure_angle_deg': 22.5, 'helix_angle_deg': 0.0}
        ],
        'bearings': [
    ...
    'rotor2': {
        'shaft': [
            {'L': 0.08, 'idl': 0.0, 'odl': 0.321, 'idr': 0.0, 'odr': 0.321},
            {'L': 0.2, 'idl': 0.0, 'odl': 0.321, 'idr': 0.0, 'odr': 0.321},
            {'L': 0.2, 'idl': 0.0, 'odl': 0.261, 'idr': 0.0, 'odr': 0.261},
            {'L': 0.64, 'idl': 0.0, 'odl': 0.261, 'idr': 0.0, 'odr': 0.261}
        ],
        'disks': [],
        'gear_elements': [
            {'n': 1, 'm': 322.0, 'Id': 24.17, 'Ip': 48.34, 'width': 0.2, 'n_teeth': 159, 'base_diameter': 0.826, 'pressure_angle_deg': 22.5, 'helix_angle_deg': 0.0}
        ],
        'bearings': [
...
def _render_tab_run():
    if not st.session_state.get("m8_loaded"):
        return st.warning("Chargez un modele.")
...
def _run_all():
    ...
    try:
        ...
        if st.session_state.get("m8_multi"):
            try:
                st.session_state["m8_modal_multi"] = st.session_state["m8_multi"].run_modal(speed=0)
            except Exception:
                pass
        try:
...
def _display_geometry():
    ...
    if multi:
        try:
            ...
            fig.update_layout(height=400, title="MultiRotor couple", margin=dict(l=0,r=0,t=40,b=0))
        ...
        with c1:
            try:
                st.plotly_chart(r1.plot_rotor().update_layout(height=300, title="Rotor 1"), use_container_width=True)
            except Exception:
                pass
        with c2:
            try:
                st.plotly_chart(r2.plot_rotor().update_layout(height=300, title="Rotor 2"), use_container_width=True)
            except Exception:
                pass
...
def _display_unbalance():
    if not st.session_state.get("m8_unbal_res"):
        return st.info("Lancez les calculs.")
